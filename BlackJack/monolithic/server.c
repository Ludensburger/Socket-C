...
This is a line of code that tells your program:
“Hey, make sure to use the Ws2_32.lib library, so I can do networking stuff like connecting to the internet.”
*/
...
void shuffleCards(int *cards, int size) {
    // pick two cards, first card is the last card in the deck, second card is a random card from the deck.
    // then swap the two cards. This process is repeated for each card in the deck.
    // the loop ensures that each iteration of the loop processes one fewer card, effectively excluding the cards that have already been swapped.
...
void fillStack(Stack *stack) {
    ...
    int cards[STACK_SIZE];
    // Set the cards' values from 0 to 51
    // meaning 0 is Ace of Spades, 1 is 2 of Spades, 2 is 3 of Spades, ..., 50 is King of Clubs, 51 is King of Diamonds
    for (int i = 0; i < STACK_SIZE; i++) {
        cards[i] = i; // Generate card values from 0 to 51
    }

    // Shuffle the cards then initialize the stack
    shuffleCards(cards, STACK_SIZE);
    initializeStack(stack);

    // Finally, push the shuffled cards into the stack
    for (int i = 0; i < STACK_SIZE; i++) {
        push(stack, cards[i]);
    }
}
